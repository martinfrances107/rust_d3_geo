...
impl<F> Resample<F> {
  pub fn new(project: Box<dyn Transform<F>>, delta2: Option<F>) -> Self
  where
    ...
    F: Float + FloatConst,
  {
    ...
    if d2 > float_4 * self.delta2.unwrap_or(F::zero()) {
      depth = depth - 1u8;
      if depth > 0u8 {
        let mut a = a0 + a1;
        let mut b = b0 + b1;
        ...
        if dz * dz / d2 > self.delta2.unwrap_or(F::zero())
