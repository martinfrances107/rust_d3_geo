...
pub fn rotate_radians<F>(delta_lambda_p: F, delta_phi: F, delta_gamma: F) -> Box<dyn Transform<F>>
  ...
  let rl: Box<dyn Transform<F>> = Box::new(RotationLambda::new(delta_lambda));
