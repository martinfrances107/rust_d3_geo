...
#[cfg(test)]
use crate::path_test_context::Path2d as WebPath2d;
...
impl Default for Path2dEndpoint {
    #[inline]
    fn default() -> Self {
        Self {
            line: LineState::Init,
...
impl Path2dEndpoint {
    ...
    #[inline]
    #[must_use]
    pub const fn new(path2d: WebPath2d) -> Self {
        Self {
            path2d,
            line: LineState::Init,
            point: PointState::Init,
...
impl PointRadiusTrait for Path2dEndpoint {
    type T = f64;
...
impl Result for Path2dEndpoint {
    type Out = WebPath2d;
    #[inline]
    fn result(&mut self) -> Self::Out {
        let mut out = WebPath2d::new().unwrap();
        mem::swap(&mut out, &mut self.path2d);
        out
    }
}

impl Stream for Path2dEndpoint {
    type EP = Self;
    type T = f64;
