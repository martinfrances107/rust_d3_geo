...
import("../pkg").then((pkg) => {
  ...
  const selectElement =
    document.querySelector<HTMLSelectElement>("#select_pattern");
  if (selectElement === null) {
    console.log("failed to find #show_rings");
    return;
  } else {
    console.log(selectElement);
    let initialSelectedPattern;
    switch (selectElement.options[selectElement.selectedIndex].value) {
      case "bar":
        initialSelectedPattern = pkg.SelectedPattern.Bar;
        break;
      case "globe":
        initialSelectedPattern = pkg.SelectedPattern.Globe;
        break;
      default:
        console.log("settings rings");
        initialSelectedPattern = pkg.SelectedPattern.Rings;
        break;
    }

    pkg.Renderer.new(initialSelectedPattern)
      .then((renderer: Renderer) => {
        ...
        const patternPromise = patternSet();
        ...
        const isSolidElem =
          document.querySelector<HTMLInputElement>("#is_solid");
        if (isSolidElem === null) {
        ...
        const dragstarted = (e: MouseEvent) => {
          ...
          selector.on("mousemove", dragged).on("mouseup", dragended);

          const canvasxy = pointer(e);
          const ep = new pkg.ExportedPoint(canvasxy[0], canvasxy[1]);
          gpos0 = renderer.invert(ep);
        };
        ...
        select(context.canvas).on("mousedown", dragstarted);
      ...
      .catch((e) => {
        console.error("Did not receive Renderer", e);
      });
  }
});
