...
pub async fn draw_equidistant(land: &Geometry<f64>) -> Result<(), JsValue> {
    let document = document()?;
    // Grab canvas.
    let canvas = document
        .get_element_by_id("equidistant-rust")
        .unwrap()
    ...
    let equidistant = Equidistant::builder()
        .scale_set::<ClipAntimeridianC<ResampleNoPCNC<Context, Equidistant, f64>, f64>>(
            width / 1.5_f64 / std::f64::consts::PI,
        )
        .rotate2_set(&[0_f64, 0_f64])
        .center_set::<ClipAntimeridianC<ResampleNoPCNC<Context, Equidistant, f64>, f64>>(&Coord {
            x: 0_f64,
            y: 0_f64,
        })
        .translate_set(&Coord {
